...
function EmailTab({ item, closeCRM }) {
  const [subject, setSubject] = useState(item.emailSubject == null ? "" : item.emailSubject);
  const [emailBody, setEmailBody] = useState(item.emailBody == null ? "" : item.emailBody);
  ...
  useEffect(() => {
    setEmailBody(item.emailBody == null ? "" : item.emailBody);
